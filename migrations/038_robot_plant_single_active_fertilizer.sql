-- 038_robot_plant_single_active_fertilizer.sql
-- Ensure only one active fertilizer effect can exist per user at a time.

-- Keep only one currently active decay_reduction effect per user (latest expiry wins).
with ranked_effects as (
  select
    id,
    row_number() over (
      partition by auth_id, effect_type
      order by expires_at desc, created_at desc, id desc
    ) as rn
  from public."RobotPlantActiveEffect"
  where effect_type = 'decay_reduction'
    and expires_at >= now()
)
delete from public."RobotPlantActiveEffect" effect
using ranked_effects ranked
where effect.id = ranked.id
  and ranked.rn > 1;
...
begin
  ...
  if v_item.item_type = 'fertilizer' then
    delete from public."RobotPlantActiveEffect"
     where auth_id = p_auth_id
       and effect_type = 'decay_reduction'
       and expires_at >= now();
  end if;
