-- Add a "source" column to get_scan_of_the_week_history so the frontend can
-- differentiate the two SOTW award paths in the UI:
--   'scheduler' = weeklyRewardsScheduler (most-liked scan, community pick -> "Wochenliebling")
--   'admin'     = AdminScanOfTheWeek.jsx (team-curated pick -> "Scan der Woche")
drop function if exists public.get_scan_of_the_week_history(integer);
...
as $$
  select
    ...
    case
      when l.event_source = 'weekly_likes_reward' then 'scheduler'
      when l.event_source = 'scan_of_the_week' then 'admin'
      else 'admin'
    end as source
