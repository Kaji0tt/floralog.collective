-- Explorer discoveries RPC with server-side audience filtering and pagination.
-- This keeps explorer results consistent across clients and supports reliable infinite loading.
...
begin
  ...
  scoped_discoveries as (
    ...
    where coalesce(d.created_date, d.discovered_date, d.updated_date) >= p_since
  )
  select d.*
  ...
  order by coalesce(d.created_date, d.discovered_date, d.updated_date) desc, d.id desc
  limit v_limit
  offset v_offset;
end;
$$;
