...
export default function FriendFriendsList() {
  ...
  useEffect(() => {
    ...
    if (resolvedProfile?.auth_id) {
      navigate(
        createPageUrl(`FriendProfile?auth_id=${encodeURIComponent(resolvedProfile.auth_id)}&tab=friends`),
        { replace: true }
      );
    }
  ...
  if (friendAuthId) {
    return (
      <Navigate
        replace
        to={createPageUrl(`FriendProfile?auth_id=${encodeURIComponent(friendAuthId)}&tab=friends`)}
      />
    );
  }
