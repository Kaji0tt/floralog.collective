...
const DialogContent = React.forwardRef(({ className, children, hideCloseButton, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onEscapeKeyDown={hideCloseButton ? (e) => e.preventDefault() : props.onEscapeKeyDown}
      onPointerDownOutside={hideCloseButton ? (e) => e.preventDefault() : props.onPointerDownOutside}
      {...props}>
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close
          className="absolute right-4 top-4 rounded-md border border-white/30 bg-black/45 p-1 text-white opacity-100 shadow-sm transition-colors hover:bg-black/65 focus:outline-none focus:ring-2 focus:ring-white/80 focus:ring-offset-2 focus:ring-offset-transparent disabled:pointer-events-none">
          <X className="h-4 w-4" />
